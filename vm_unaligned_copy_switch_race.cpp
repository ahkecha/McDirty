...
static const char* _target_file;
static const char* _overwrite_file;
    ...
    "Test that unaligned copy respects read-only mapping")
{
...
#endif
	int fd = open(_target_file, O_RDONLY | O_CLOEXEC);
	...
	FILE* overwrite_file = fopen(_overwrite_file, "r");
	if (overwrite_file == NULL)
	{
		std::cerr << "[!] Overwrite file not found !" << std::endl;
		exit(EXIT_FAILURE);
	}
	...
	{
		std::cerr << "[!] Overwrite file too long!" << std::endl;
		exit(EXIT_FAILURE);
	}
	if (!overwrite_length)
		std::cerr << "[!] Warning: Overwrite file is empty !" << std::endl;
...
int main(int ac, char **av)
{
	if (ac != 3)
	{
		std::cerr << "Usage: ./a.out [target_file] [overwrite_file]" << std::endl;
		exit(EXIT_FAILURE);
	}
	_target_file = av[1];
	_overwrite_file = av[2];
	std::cout << "[+] Press Enter to fire in the hole" << std::endl;
	std::cin.ignore();
